<template>
  <header class="db w-100 border-box ph5-l ph3" :class="[lights ? 'z-3' : 'z-max', {'alt-header': routeName === 'series' || routeName === 'media', 'box-shadow-umi': routeName !== 'media'}]">
...
export default {
  ...
  computed: {
    ...
    connectedCount() {
      ...
      return this.roomHover || this.roomMenu
...
header {
  ...
  transform: translateZ(
    0
  ); /* hack fix for 1px jitter when a transform happens on the page */
}
...
.alt-header .menu-circle:hover,
.alt-header .menu-circle.active {
  color: transparent;
}

.alt-header .counter,
.media-header .counter {
  background-color: #fff;
  color: #333;
}

.alt-header .counter:after,
.media-header .counter:after {
  border-left-color: #fff;
}

.alt-header .nav a:hover,
.alt-header .nav .router-link-active {
